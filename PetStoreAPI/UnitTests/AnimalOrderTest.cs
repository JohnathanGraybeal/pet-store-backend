...
namespace UnitTests
{
    public class AnimalOrderTests
    {
        ...
        {
            //Arrange
            var fakeRevision = A.Dummy<AnimalOrder>();
            var dataStore = A.Fake<IAnimalOrderRepository>();
            A.CallTo(() => dataStore.CreateAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new AnimalOrderController(dataStore);
            ...
            Assert.AreEqual(0, result.RouteValues["orderId"]);   // Checks that the Id of the entry is what was submitted.
        }
        ...
        public async Task GetAll_Returns_All_Values()
        {
            //Arrange
            int count = 3;
            var fakeRevisions = A.CollectionOfDummy<AnimalOrder>(count).AsEnumerable();
            var dataStore = A.Fake<IAnimalOrderRepository>();
            ...
            var returnRevisions = result.Value as IEnumerable<AnimalOrder>;
        ...
        public async Task GetOne_Gets_Same_ID()
        {
            //Arrange
            int Id = 1;
            var fakeRevision = A.Fake<AnimalOrder>();
            fakeRevision.Id = Id;
            var fakeRevision2 = A.Fake<AnimalOrder>();
            fakeRevision2.Id = 2;
            var dataStore = A.Fake<IAnimalOrderRepository>();
