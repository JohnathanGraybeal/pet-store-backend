...
namespace UnitTests
{
    public class CustomerAccountTests
    {
        ...
        {
            //Arrange
            var fakeRevision = A.Dummy<CustomerAccount>();
            var dataStore = A.Fake<ICustomerAccountRepository>();
            A.CallTo(() => dataStore.CreateAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new CustomerAccountController(dataStore);
        ...
        public async Task GetAll_Returns_All_Values()
        {
            //Arrange
            int count = 3;
            var fakeRevisions = A.CollectionOfDummy<CustomerAccount>(count).AsEnumerable();
            var dataStore = A.Fake<ICustomerAccountRepository>();
            ...
            var returnRevisions = result.Value as IEnumerable<CustomerAccount>;
        ...
        public async Task GetOne_Gets_Same_ID()
        {
            //Arrange
            int Id = 1;
            var fakeRevision = A.Fake<CustomerAccount>();
            fakeRevision.Id = Id;
            var fakeRevision2 = A.Fake<CustomerAccount>();
            fakeRevision2.Id = 2;
            var dataStore = A.Fake<ICustomerAccountRepository>();
