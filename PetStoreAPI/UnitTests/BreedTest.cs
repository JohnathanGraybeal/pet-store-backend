...
namespace UnitTests
{
    public class BreedTests
    {
        ...
        [Test]
        public async Task Create_Adds_Breed()
        {
            //Arrange
            var fakeRevision = A.Dummy<Breed>();
            var dataStore = A.Fake<IBreedRepository>();
            A.CallTo(() => dataStore.CreateAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new BreedController(dataStore);
        ...
        public async Task GetAll_Returns_All_Values()
        {
            //Arrange
            int count = 3;
            var fakeRevisions = A.CollectionOfDummy<Breed>(count).AsEnumerable();
            var dataStore = A.Fake<IBreedRepository>();
            ...
            var returnRevisions = result.Value as IEnumerable<Breed>;
        ...
        /// <returns>OkObjectResult</returns>
        [Test]
        public async Task GetOne_Gets_Same_Breed()
        {
            //Arrange
            var fakeRevision = A.Fake<Breed>();
            fakeRevision.Category = "Bird";
            var fakeRevision2 = A.Fake<Breed>();
            fakeRevision2.Category = "Bird";
            var dataStore = A.Fake<IBreedRepository>();
            A.CallTo(() => dataStore.ReadAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new BreedController(dataStore);

            //Act
            var actionResult = await controller.GetOne(fakeRevision);

            //Assert
            var result = actionResult.Result as OkObjectResult;
            var returnRevision = result.Value;
            Assert.IsNotNull(returnRevision);   // Checks that returned entry is not null
            Assert.AreEqual(fakeRevision, result.Value);    // Checks that returned entry is the entry with given breed
        }
        ...
        public async Task Put_Updates_Entry()
        {
            ...
            var fakeRevision = A.Dummy<Breed>();
            var dataStore = A.Fake<IBreedRepository>();
            A.CallTo(() => dataStore.UpdateAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new BreedController(dataStore);
        ...
        public async Task Remove_deletes_Entry()
        {
            ...
            var fakeRevision = A.Dummy<Breed>();
            var dataStore = A.Fake<IBreedRepository>();
            A.CallTo(() => dataStore.DeleteAsync(fakeRevision)).Returns(Task.FromResult(fakeRevision));
            var controller = new BreedController(dataStore);

            //Act
            var actionResult = await controller.Remove(fakeRevision);
